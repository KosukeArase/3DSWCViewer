...
def process():
    with open('swc/'+file, 'r') as f:
        ...
        for i in range(N):
            ...
            if x+y>thresh_xy and z < thresh_z:
                coordinates[3,i] = 1
            # if toroid_flag == 7.:
            #     coordinates[4,i] = 1


    og_lower = np.where(coordinates[3]==1) # og_lower
    other = np.where(coordinates[3]==0) # others
    with open(file.split('.')[0]+'_og_lower.txt', 'w') as f:
        for syn in og_lower[0]:
            f.write(str(syn+1)+'\n')

    _trace1 = get_trace(coordinates[0][og_lower], coordinates[1][og_lower], coordinates[2][og_lower], 'rgb(255, 0, 0)') #og lower
    _trace2 = get_trace(coordinates[0][other], coordinates[1][other], coordinates[2][other], 'rgb(0, 0, 255)') # other
